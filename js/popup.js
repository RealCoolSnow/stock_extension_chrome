...
function setPriceDisplay() {
	if (backgroundPage.Settings.getValue("showPrice", true)) {
		...
		backgroundPage.Settings.setValue("showRange", false);
...
function createStockDisplayDOM() {
	...
	if (backgroundPage.Settings.getValue("showSinaLink", true) || backgroundPage.Settings.getValue("showXueqiuLink", true))
	...
	else
	{
		...
		for (var i in stockGetInfos) {
			...
			html += "		</div>\r\n";
			if (backgroundPage.Settings.getValue("showSinaLink", false) || backgroundPage.Settings.getValue("showXueqiuLink", false))
		...
		$("#stockDisplay").css("height", 320 + Math.ceil(stockGetInfos.length/6) * 34);
...
function displayPopupStocks() {
	stockGetInfos = backgroundPage.stockGetInfos;
	
	for (var i in stockGetInfos) {
		var stockInfo = stockGetInfos[i];
		displayPopupStock(stockInfo);
	}

	window.setTimeout(displayPopupStocks, 4000);
}

function displayPopupStock(stockInfo) {
	try {
		var color = "#333";
